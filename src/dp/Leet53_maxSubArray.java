...
public class Leet53_maxSubArray {
    public static void main(String[] args) {
        int i = new Leet53_maxSubArray().maxSubArray(new int[]{-2, 1, -3, 4, -1, 2, 1, -5, 4});
        return;
    }

    public int maxSubArray(int[] nums) {
        int maxEndingHere = nums[0];
        int maxEndingLast = nums[0];
        for (int i = 1; i < nums.length; i++) {
            maxEndingHere = Math.max(nums[i], nums[i] + maxEndingHere);
            maxEndingLast = Math.max(maxEndingHere, maxEndingLast);
        }
        return maxEndingLast;
    }
}
